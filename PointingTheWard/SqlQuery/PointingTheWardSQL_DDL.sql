...
//그룹 삭제시 그룹에 포함된 인원 모두 삭제
CREATE OR REPLACE TRIGGER GROUP_DELETE_TRIGGER
BEFORE DELETE ON GROUP_TB
FOR EACH ROW
BEGIN
DELETE FROM G_PARTICIPANT_VIEW WHERE G_CODE = :OLD.G_CODE OR EMAIL = :OLD.OWNER
END;


//이벤트 참여목록을 삭제한 후 해당 코드를 가진 인원이 없을 시 이벤트 삭제
...
BEGIN
SELECT COUNT(*) INTO E_PARTICIPANT_CNT FROM E_PARTICIPANT_VIEW WHERE E_CODE = :OLD.E_CODE
IF 
E_PARTICIPANT_CNT = 0 
THEN 
DELETE FROM EVENT_VIEW WHERE E_CODE = :OLD.E_CODE
END;

//그룹 참여 목록을 삭제 한 후 해당 코드를 가진 인원이 없을 시 그룹 삭제
...
BEGIN
SELECT COUNT(*) INTO G_PARTICIPANT_CNT FROM G_PARTICIPANT_VIEW WHERE G_CODE = :OLD.G_CODE
IF G_PARTICIPANT_CNT = 0 
THEN DELETE FROM GROUP_VIEW WHERE G_CODE = :OLD.G_CODE
END;
...
FOR EACH ROW
BEGIN
DELETE FROM GROUP_VIEW WHERE owner = :OLD.EMAIL
DELETE FROM E_PARTICIPANT_VIEW WHERE EMAIL = :OLD.EMAIL
...
FOR EACH ROW
BEGIN
INSERT INTO E_PARTICIPANT_VIEW(E_CODE,EMAIL)VALUES(:NEW.E_CODE,':NEW.EMAIL')
END;
